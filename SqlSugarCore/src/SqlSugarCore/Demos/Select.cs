        ...
        private void QueryableDemo()
        {
            ...
            using (var db = SugarDao.GetInstance())
            {
               ...
               .ToDynamic();
                ...
                var list5= queryable2.Select<Student, School, Area, classNew>((s1, s2, a1) => new classNew { newid = s1.id, studentName = s1.name, schoolName = s2.name, areaName = a1.name })
                    .ToPageList(0, 200);

                var count2 = queryable2.Count();
            }
        }
    }
}
